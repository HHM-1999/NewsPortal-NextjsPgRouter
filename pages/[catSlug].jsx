...
export function getServerSideProps(context) {
    ...
    return getApi(`category/${catSlug}`)
      .then((CategoryList) => {
        const category = CategoryList?.category;
        if (!category) return { notFound: true };
  
        const catID = category.CategoryID;
        return getApi(`inner-category-content/${catID}/6`).then((leadNews) => {
          ...
          return postApi(`inner-category-content-more`, formData).then((newsResponse) => {
      ...
      .catch((err) => {
        console.error("CategoryPage server error:", err);
